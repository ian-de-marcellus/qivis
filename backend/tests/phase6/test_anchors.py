...
class TestAnchorAPI:
    ...
    async def test_is_anchored_reflects_state(self, client):
        ...
        tree = await create_rhizome_with_messages(client, n_messages=2)
        ...
        resp = await client.post(
            f"/api/rhizomes/{rhizome_id}/nodes/{user_node_id}/anchor"
        )
        assert resp.status_code == 200
    ...
    async def test_toggle_unanchors(self, client):
        ...
        # Anchor
        resp = await client.post(f"/api/rhizomes/{rhizome_id}/nodes/{node_id}/anchor")
        assert resp.json()["is_anchored"] is True
...
class TestBulkAnchor:
    """POST /api/rhizomes/{rhizome_id}/bulk-anchor endpoint."""

    async def test_bulk_anchor_multiple_nodes(self, client):
        """Bulk anchor creates anchors for multiple nodes."""
        tree = await create_rhizome_with_messages(client, n_messages=4)
        rhizome_id = tree["rhizome_id"]
        node_ids = tree["node_ids"]

        resp = await client.post(
            f"/api/rhizomes/{rhizome_id}/bulk-anchor",
    ...
    async def test_bulk_anchor_skips_already_anchored(self, client):
        ...
        for nid in node_ids[:2]:
            await client.post(f"/api/rhizomes/{rhizome_id}/nodes/{nid}/anchor")
    ...
    async def test_bulk_anchor_404_for_nonexistent_tree(self, client):
        ...
        resp = await client.post(
            "/api/rhizomes/nonexistent/bulk-anchor",

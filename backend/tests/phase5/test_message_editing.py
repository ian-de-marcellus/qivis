...
class TestNodeContentEditedProjection:
    ...
    async def test_edit_sets_edited_content(self, event_store, projector):
        ...
        node_ev = make_node_created_envelope(
            rhizome_id=tree_ev.rhizome_id, content="Original text",
        )
        ...
        nodes = await projector.get_nodes(tree_ev.rhizome_id)
        assert len(nodes) == 1
    ...
    async def test_restore_clears_edited_content(self, event_store, projector):
        ...
        tree_ev = make_rhizome_created_envelope()
        node_ev = make_node_created_envelope(
            rhizome_id=tree_ev.rhizome_id, content="Original",
        )
        ...
        # Edit
        edit_ev = make_node_content_edited_envelope(
            rhizome_id=tree_ev.rhizome_id, node_id=node_id,
    ...
    async def test_edit_preserves_original_content(self, event_store, projector):
        ...
        tree_ev = make_rhizome_created_envelope()
        node_ev = make_node_created_envelope(
            rhizome_id=tree_ev.rhizome_id, content="Immutable original",
        )
    ...
    async def test_edit_does_not_affect_other_nodes(self, event_store, projector):
        ...
        node_a = make_node_created_envelope(
            rhizome_id=tree_ev.rhizome_id, content="Node A",
        )
        node_b = make_node_created_envelope(
            rhizome_id=tree_ev.rhizome_id, content="Node B",
        )
...
class TestEditNodeContentAPI:
    """PATCH /api/rhizomes/{rhizome_id}/nodes/{node_id}/content"""
    ...
    async def test_patch_nonexistent_tree_404(self, client):
        resp = await client.patch(
            "/api/rhizomes/no-such-tree/nodes/no-such-node/content",
    ...
    async def test_patch_nonexistent_node_404(self, client):
        ...
        resp = await client.patch(
            f"/api/rhizomes/{rhizome_id}/nodes/no-such-node/content",
            json={"edited_content": "x"},
        )
        assert resp.status_code == 404
...
class TestGetEventsByType:
    ...
    async def test_filters_by_event_type(self, event_store, projector):
        ...
        tree_ev = make_rhizome_created_envelope()
        node_ev = make_node_created_envelope(
            rhizome_id=tree_ev.rhizome_id, content="Hello",
        )
        edit_ev = make_node_content_edited_envelope(
            rhizome_id=tree_ev.rhizome_id,
            node_id=node_ev.payload["node_id"],
        ...
        results = await event_store.get_events_by_type(
            tree_ev.rhizome_id, "NodeContentEdited",
        )
    ...
    async def test_scoped_to_tree(self, event_store, projector):
        ...
        tree1 = make_rhizome_created_envelope()
        tree2 = make_rhizome_created_envelope()
        node1 = make_node_created_envelope(rhizome_id=tree1.rhizome_id, content="A")
        node2 = make_node_created_envelope(rhizome_id=tree2.rhizome_id, content="B")
        ...
        edit1 = make_node_content_edited_envelope(
            rhizome_id=tree1.rhizome_id, node_id=node1.payload["node_id"],
            original_content="A", new_content="A edited",
        )
        edit2 = make_node_content_edited_envelope(
            rhizome_id=tree2.rhizome_id, node_id=node2.payload["node_id"],
        ...
        results = await event_store.get_events_by_type(
            tree1.rhizome_id, "NodeContentEdited",
        )
        assert len(results) == 1
...
class TestGetEditHistory:
    """RhizomeService.get_edit_history returns correct edit history."""
    ...
    async def test_empty_for_unedited_node(self, client):
        ...
        resp = await client.get(
            f"/api/rhizomes/{rhizome_id}/nodes/{node_id}/edit-history",
        )
        assert resp.status_code == 200
        body = resp.json()
    ...
    async def test_scoped_to_node(self, client):
        ...
        await client.patch(
            f"/api/rhizomes/{rhizome_id}/nodes/{node_a}/content",
            json={"edited_content": "Edit A"},
        )
        await client.patch(
            f"/api/rhizomes/{rhizome_id}/nodes/{node_b}/content",
            json={"edited_content": "Edit B"},
        )

        resp = await client.get(
            f"/api/rhizomes/{rhizome_id}/nodes/{node_a}/edit-history",
        )
        body = resp.json()
        assert len(body["entries"]) == 1
    ...
    async def test_nonexistent_tree_404(self, client):
        resp = await client.get(
            "/api/rhizomes/no-such-tree/nodes/no-such-node/edit-history",
        )
        assert resp.status_code == 404
    ...
    async def test_nonexistent_node_404(self, client):
        tree = await create_test_rhizome(client)
        resp = await client.get(
            f"/api/rhizomes/{tree['rhizome_id']}/nodes/no-such-node/edit-history",
        )
        assert resp.status_code == 404

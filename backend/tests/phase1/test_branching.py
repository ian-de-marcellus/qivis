...
from tests.fixtures import create_branching_rhizome, create_test_rhizome
...
class TestSiblingMetadata:
    ...
    async def test_two_siblings_have_correct_count_and_indices(self, client):
        ...
        # Parent node
        parent = await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={
            "content": "Parent",
        })
    ...
    async def test_siblings_at_different_depths_independent(self, client):
        ...
        root = await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={
            "content": "Root",
        })
        root_id = root.json()["node_id"]

        child_a = await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={
            "content": "A",
            "role": "assistant",
            "parent_id": root_id,
        })
    ...
    async def test_create_node_response_includes_sibling_fields(self, client):
        """POST /api/rhizomes/{id}/nodes response has sibling_count and sibling_index."""
        tree = await create_test_rhizome(client)
        rhizome_id = tree["rhizome_id"]
    ...
    async def test_tree_detail_nodes_include_sibling_fields(self, client):
        """GET /api/rhizomes/{id} nodes all have sibling_count and sibling_index."""
        tree = await create_test_rhizome(client)
        rhizome_id = tree["rhizome_id"]

        await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={"content": "A"})
        await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={"content": "B"})
...
class TestBranchingTreeStructure:
    ...
    async def test_create_branch_from_non_leaf_node(self, client):
        ...
        # Linear: A -> B -> C
        a = await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={
            "content": "A",
        })
        a_id = a.json()["node_id"]

        b = await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={
            "content": "B",
            "role": "assistant",
            "parent_id": a_id,
        })
        ...
        d = await client.post(f"/api/rhizomes/{rhizome_id}/nodes", json={
            "content": "D (fork)",
            "role": "assistant",
            "parent_id": a_id,
        })
    ...
    async def test_deep_branching_tree_sibling_counts(self, client):
        """Verify topology from create_branching_rhizome fixture."""
        result = await create_branching_rhizome(client)
        rhizome_id = result["rhizome_id"]
        ids = result["node_ids"]
